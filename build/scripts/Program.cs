...
namespace Build
{
    ...
    <PropertyGroup>
        <VersionPrefix>{gitVersion.Version}</VersionPrefix>
            ...
            Target("build", () =>
            {
                RunShell($"dotnet build {commandBuildArgs} {ExpandPath("./AptRepoBuilder.sln")}");
            });

            Target("deploy", () =>
            {
                RunShell($"dotnet pack --output {ExpandPath("./output")} {commandBuildArgsWithVersion} {ExpandPath("./src/AptRepoBuilder/AptRepoBuilder.csproj")}");
            });
